...
calcular_holt_winters <- function(periodos, valores, frecuencia, forecast_periods, alpha = NULL, beta = NULL, gamma = NULL) {
  ...
  # Predicción para los períodos indicados
  predicciones <- forecast(modelo_hw, h = forecast_periods)$mean
  ult_periodo <- max(periodos)
  periodos_prediccion <- (ult_periodo + 1):(ult_periodo + forecast_periods)
  ...
  df_prediccion <- data.frame(time = periodos_prediccion, value = predicciones)
  ...
  grafica <- ggplot() +
    ...
    geom_point(data = df_prediccion, aes(x = time, y = value), color = "red", size = 3) +
  ...
  # Imprimir la predicción para los próximos períodos
  cat("Predicciones para los próximos", forecast_periods, "períodos:", predicciones, "\n")
}
...
calcular_holt_winters(periodos, valores, frecuencia = 4, forecast_periods = 5, alpha = 0.8, beta = 0.2, gamma = 0.2)